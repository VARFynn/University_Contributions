################# Advanced Macroeconometrics - Assignment 4 #################
...
# outside loop calculations to save computation time
XX <- crossprod(X)
...
#autoregressive coefficients: A_store[,ii,jj] -> ii... number of exogenous variables, jj... number of endogenous variables
a_coef <- as.data.frame(matrix(NA, nrow=k, ncol=M)) 
for (ii in 1:M) {
  for (jj in 1:k) {
    a_coef[jj,ii] <- paste0(round(mean(A_store[,jj,ii]),3), " [", round(quantile(A_store[,jj,ii], 0.1), 3), ", ",   
                            round(quantile(A_store[,jj,ii], 0.90), 3), "]")
  } }

names(a_coef) <- c(colnames(Y))
row.names(a_coef) <- c(paste0(colnames(Y),"_l1"), paste0(colnames(Y),"_l2"), paste0(colnames(Y),"_l3"), paste0(colnames(Y),"_l4"), "Constant")
stargazer(a_coef,out="C:/Users/elisa/Dropbox/Advanced Macroeconometrics/Assignments/Homework 4/Figures/Table1.tex",summary=F,align = T, title="Table 1", digits=4,type="latex")
stargazer(a_coef,out="C:/Users/elisa/Dropbox/Advanced Macroeconometrics/Assignments/Homework 4/Figures/Table1.html",summary=F,align = T, title="Table 1", digits=4)
...
names(a_coef) <- c(colnames(Y))
row.names(a_coef) <- c(paste0(colnames(Y),"_l1"), paste0(colnames(Y),"_l2"), paste0(colnames(Y),"_l3"), paste0(colnames(Y),"_l4"), "Constant")
stargazer(a_coef,out="C:/Users/elisa/Dropbox/Advanced Macroeconometrics/Assignments/Homework 4/Figures/Table2.tex",summary=F,align = T, title="Table 2", digits=4,type="latex")
stargazer(a_coef,out="C:/Users/elisa/Dropbox/Advanced Macroeconometrics/Assignments/Homework 4/Figures/Table2.html",summary=F,align = T, title="Table 2", digits=4)
...
stargazer(a_coef,out="C:/Users/elisa/Dropbox/Advanced Macroeconometrics/Assignments/Homework 4/Figures/Table3.tex",summary=F,align = T, title="Table 3", digits=4,type="latex")
stargazer(a_coef,out="C:/Users/elisa/Dropbox/Advanced Macroeconometrics/Assignments/Homework 4/Figures/Table3.html",summary=F,align = T, title="Table 3", digits=4)
...
stargazer(a_coef,out="C:/Users/elisa/Dropbox/Advanced Macroeconometrics/Assignments/Homework 4/Figures/Table4.tex",summary=F,align = T, title="Table 4", digits=4,type="latex")
stargazer(a_coef,out="C:/Users/elisa/Dropbox/Advanced Macroeconometrics/Assignments/Homework 4/Figures/Table4.html",summary=F,align = T, title="Table 4", digits=4)
...
plot_errors <- function(ts_data, title) {
  plot(ts_data, main = paste("Error", title), xlab = "Years", las=3, lwd = 2)
  abline(h = 0, lty = "dashed", col = "black")
}

titles <- c("delta_oil_production", "real_activity", "real_price_oil")

for (title in titles) {
  plot_errors(ts_delta_oil_prod, title)
}
...
# Replicating Figure 3 with Sign Restrictions ----------------------------
set.seed(1)
for (i in 1:3) {
...
# Unemployment and oil supply shocks -----------------------------------------------
rm(list=ls())
pacman::p_load(
  tidyverse,
  urca,
...
cons <- TRUE  # include constant?
# Create data matrices
data_with_unemployment <- read_excel("Data_with_unemployment.xlsx")
data <- data.frame(data_with_unemployment$delta_oil_prod, data_with_unemployment$real_activity,
                   data_with_unemployment$real_price_oil, data_with_unemployment$unrate)
library(tseries)
adf.test(data$unrate)
...
{# let's have a look at OLS estimates
  ...
  axis(1, at=seq(1,395,by=20), labels=time[seq(1,395,by=20)], las=2)
  abline(v=seq(1,395,by=20), col="lightgrey", lty=3)
  plot.ts(Y[,4], xlab="", ylab="", main="Unemployment Rate", xaxt="n", lwd=2, cex.main=1.5, cex.lab=2)
...
for(irep in 1:ntot){
  ...
  if(irep > nburn){
    ...
    while(cond.overall && counter < MaxTries){
      ...
      # Oil supply shock shock
      cond.SP <- (shock.sign[1, 3] > 0) * (shock.sign[2, 3] > 0) * (shock.sign[3, 3] > 0) * (shock.sign[4, 3] < 0)
      # Aggregate demand shock
      cond.AD <- (shock.sign[1, 1] < 0) * (shock.sign[2, 1] > 0) * (shock.sign[3, 1] > 0) * (shock.sign[4, 1] > 0)
      # Oil-specific demand shock
      cond.OSD <- (shock.sign[1, 2] < 0) * (shock.sign[2, 2] > 0) * (shock.sign[3, 2] < 0) * (shock.sign[4, 2] < 0)
      # Unemployement shock
      cond.UN <- (shock.sign[1, 4] > 0) * (shock.sign[2, 4] < 0) * (shock.sign[3, 4] < 0) * (shock.sign[4, 4] > 0)
      
      #Shocks have to be mutually exclusive (orthogonal)
      cond.overall <- (cond.MP * cond.AS * cond.AD * cond.UN)==0
    }
    cou_store[irep - nburn] <- counter
...
# Identification using short-run restrictions -----------------------------
par(mfrow=c(4,4),mar=c(4,4,2,2))
for(ii in 1:M){
  for(jj in 1:M){
    min1 <- min(IRFsign_low[ii,jj,])
